...
export function HomeHero({ locale }: { locale: PublicLocale }) {
  ...
  return (
    <>
      ...
      <section
        className="home-cinematic-hero relative isolate h-[100svh] overflow-hidden bg-[#050605] text-white"
        id="home"
        ref={rootRef}
      >
        ...
        <div
          className="relative z-10 mx-auto grid h-full max-w-[94rem] grid-cols-1 grid-rows-[auto_1fr] px-5 pb-5 pt-24 [direction:ltr] md:px-8 md:pt-28 lg:grid-cols-[0.92fr_1.08fr] lg:grid-rows-1 lg:gap-5 lg:px-10 lg:pb-5 lg:pt-24 xl:px-14"
        >
          <div
            className={`relative z-20 order-1 flex flex-col justify-center py-4 ${
              locale === 'ar' ? 'lg:order-2 lg:py-16 lg:ps-8' : 'lg:order-1 lg:py-4 xl:py-8'
            }`}
          ...
          >
            ...
            <div
              className={`mt-5 max-w-xl border border-white/12 bg-white/[0.035] p-4 backdrop-blur-md ${
                locale === 'ar' ? 'lg:mt-7' : 'lg:mt-5'
              }`}
              data-hero-copy
            >
              <div className="flex items-center justify-between border-b border-white/10 pb-3">
                ...
                <span className="text-[0.5rem] font-bold uppercase tracking-[0.16em] text-white/24">System / 02.07</span>
              </div>
              <div className="grid grid-cols-3 pt-4">
                {[
                  ...
                  <div
                    className={`min-w-0 ${metricIndex > 0 ? 'border-s border-white/10 ps-4' : 'pe-4'}`}
                    key={String(label)}
                  >
                    <div className="flex items-end justify-between gap-2">
                      <p className="text-2xl font-black tracking-[-0.06em] text-white">{String(value)}</p>
                      <span className="mb-1 text-[0.48rem] font-black text-[#39ff14]">+{metricIndex + 3}.2%</span>
                    </div>
                    <p className="mt-1 truncate text-[0.58rem] font-semibold text-white/38">{String(label)}</p>
          ...
          <div
            className={`relative z-10 order-2 min-h-[16rem] md:min-h-[22rem] lg:min-h-0 ${
              locale === 'ar' ? 'lg:order-1' : 'lg:order-2'
            }`}
            data-bolt-stage
          >
            <div className="absolute inset-y-[2%] left-1/2 w-[116%] -translate-x-1/2 lg:inset-y-[5%] lg:w-[112%]">
              <div
                aria-hidden="true"
